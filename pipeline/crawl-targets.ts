...
function readTargetDomains(inputPath: string): string[] {
  const text = require("node:fs").readFileSync(inputPath, "utf-8") as string;
  return text
    .split("\n")
    .map((l: string) => l.trim())
...
async function main() {
  ...
  if (exitCode !== 0) {
    usageAndExit(`cariddi exited with code ${exitCode}. stderr:\n${stderr}`);
  }
  ...
  const findingsHeader = [
    ...
    `# Source: ${args.input}`,
    `# ${crawledCount} URL(s) crawled, ${secretsCount} with secrets found, ${followupCount} flagged for Strix follow-up (errors/info, no secrets)`,
    "",
  ].join("\n");
  ...
  console.log(`Crawled ${crawledCount} URL(s).`);
