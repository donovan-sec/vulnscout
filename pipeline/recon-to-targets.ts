...
function parseArgs(argv: string[]) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === "--program") {
      ...
      if (!value) usageAndExit("--program requires a non-empty value");
      args.program = value;
...
// A hostname regex is the scope gate's last line of defense against a
// recon domain string that isn't actually a single, well-formed hostname --
// e.g. one carrying an embedded newline, control characters, a URL scheme,
// a path, or a port. Matching/writing an unvalidated string let a single
// scope-matching suffix drag an arbitrary second "domain" (smuggled via
// \n) into the target list (Forge review, 2026-07-21).
const HOSTNAME_RE =
  /^(?!-)[a-z0-9-]{1,63}(?<!-)(\.(?!-)[a-z0-9-]{1,63}(?<!-))*\.?$/;

function isValidHostname(value: string): boolean {
  if (!value || value.length > 253) return false;
  return HOSTNAME_RE.test(value.toLowerCase());
}
...
async function main() {
  ...
  const droppedInvalidHostname: string[] = [];

  for (const profile of profiles) {
    if (!isValidHostname(profile.domain)) {
      droppedInvalidHostname.push(profile.domain);
      continue;
    }
  ...
  const header = [
    ...
    `# ${kept.length} target(s), ${droppedBelowThreshold.length} dropped below threshold, ${droppedOutOfScope.length} dropped out of scope, ${droppedInvalidHostname.length} dropped invalid hostname`,
    "",
  ].join("\n");
  await Bun.write(outPath, header + kept.join("\n") + (kept.length ? "\n" : ""));

  console.log(`Wrote ${kept.length} target(s) to ${outPath}`);
  if (droppedInvalidHostname.length) {
    console.log(`Dropped (not a valid single hostname): ${droppedInvalidHostname.join(", ")}`);
  }
